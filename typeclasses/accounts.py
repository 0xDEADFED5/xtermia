...
class Account(DefaultAccount):
    ...
    * Hook methods (when re-implementation, remember methods need to have self as first arg)
    ...
    def at_post_login(self, session=None, **kwargs):
        ...
        # (don't) inform the client that we logged in through an OOB message
        # if session:
        #     session.msg(logged_in={})

        self._send_to_connect_channel(gettext("|G{key} connected|n").format(key=self.key))
        if settings.AUTO_PUPPET_ON_LOGIN:
            # in this mode we try to auto-connect to our last connected object, if any
            try:
                self.puppet_object(session, self.db._last_puppet)
            except RuntimeError:
                self.msg(gettext("The Character does not exist."))
                return
        else:
